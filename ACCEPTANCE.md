...
- 도구 이름과 실제 실행 결과: `read_file` 2회(A01), `write_file` 2회 중 1회 `invalid_argument`·1회 `rejected_by_user`(A04) — 세션 파일: `evidence_sessions/20260908T062527Z-dd5ca30f.jsonl`, `evidence_sessions/20260908T062548Z-9435a1fc.jsonl`, `evidence_sessions/20260908T062825Z-a11b62b6.jsonl`
...
**실제 실행(2026-09-08, 제출 후 추가)**: `examples/eval/작업자자료.jsonl`의 `w02` 사용 — 입력 "나는 어제 국어공부 했어", 실제 엔진이 낸 (틀린) 결과 "나는 어제 국어공부했어"(정답은 "나는 어제 국어 공부했어"). 이 사례를 직접 고르기 전에 `.venv`로 실제 71건을 돌려 현재 진짜로 틀리는 11건을 먼저 찾았다(`heldout.jsonl` 41건은 전부 PASS, `작업자자료.jsonl`에서만 11건 불일치 — 스크립트와 전체 목록은 `evidence_sessions/README.md`에 정리).

- (1) 충족: `search_text` 2회, `read_file` 2회 실행(`evidence_sessions/20260908T083416Z-553a604d.jsonl`).
- (2) 충족: 최종 답이 인용한 `tests/test_josa_affix.py:234`는 실재하는 파일이고 실재하는 줄(총 237줄 중)이다.
- (3) 부분 충족 — **정답과 대조**: 모델 답이 인용한 위치는 **테스트 파일**이었다. 독립적으로 코드를 읽어 확인한 진짜 원인은 `subtitle_corrector/docs/BACKLOG.md`(42행)·`docs/IMPLEMENTATION_LOG.md`(237~256행)에 문서화된 **의도된 미구현**이다 — `국어공부`처럼 사전에 없는 명사쌍은 `word_exists`·`compound_status` 같은 사전 신호만으로는 붙여야 할지 띄워야 할지 가를 수 없어서(`특수제작했습니다`는 정반대로 완전히 떼야 함) 자동화를 보류했다고 개발자가 직접 적어 뒀다. 모델의 **추론 내용 자체는 이 설명과 사실상 일치**했다(같은 문장 인용: "사전에 아예 없는 명사쌍은 이 규칙의 근거가 없어 건드리지 않는다") — 하지만 인용한 `파일:줄`은 그 설명이 적힌 **테스트 코드**이지, 실제로 그 판단을 실행하는 **엔진 코드**(`subtitle_corrector/engine/affix.py`의 `correct_action_noun_affix`/`correct_noun_phrase_hada_detach`)가 아니었다.
- 원인 추적: 세션 기록을 보면 모델은 `국어공부`라는 **문자열 그대로**만 검색했다(`search_text 국어공부 **/*.py`). 이 문자열은 일반화된 엔진 코드 어디에도 나오지 않고(엔진은 특정 단어를 하드코딩하지 않는다), 마침 그 사례를 언급한 테스트 docstring에만 등장해서 거기로만 연결됐다 — read_file로 확장 검색을 하거나 "affix", "동작성", "compound_status" 같은 개념어로 다시 검색했다면 엔진 소스에 닿았을 가능성이 있는데 그렇게 하지 않고 답을 냈다.
- 결론: **하네스는 정직하게 동작했다**(실재하는 근거만 인용, 지어내지 않음) **하지만 모델의 답은 "정답 파일"이 아니라 "정답과 같은 내용을 설명하는 다른 파일"을 짚었다** — 최종 판정은 FAIL(엄격 기준: 실행 코드 위치를 못 맞힘) 또는 PARTIAL(느슨한 기준: 원인 설명 자체는 맞음)이며, 이 프로젝트는 엄격 기준을 채택해 **S01: FAIL**로 기록한다. 그래도 하네스가 실재하지 않는 근거를 지어내지 않았다는 점, 관련 문서를 실제로 찾아 정확히 인용했다는 점은 유지된다.
