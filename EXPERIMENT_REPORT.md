...
|---|---|---|---|
| `read_file`로 실제 자료를 읽고 답함 | "추측하지 말고 실제로 읽은 근거로 답해라" | A01: `examples/sample.srt`(10줄) 요약 요청 → `read_file` 1회 실행, 반환된 `content`가 원본과 문자 단위로 일치, 최종 답 10개 항목 전부 원문 문장에 대응, 없는 내용 추가 없음(`harness/.sessions/20260908T062527Z-dd5ca30f.jsonl`) | 충족 |
...
한계로 남긴 것: A08(같은 세션 후속 요청, CLI 미노출), A10(Ollama 어댑터, D08 CHOSEN이지만 미실행)은 NOT_RUN으로 정직하게 남겼다. 벤치마크(R08) 결과는 이 제품 요구의 충족 여부와는 별개다 — 아래 "실패 분석과 개선 가설"은 R08 자체의 실험이며, 자막 교정기 제품 기능의 검증은 위 표로 완결된다.
...
- 실제 변화: 예상대로 sudoku가 **처음으로 완전 통과**(22/22)했고, sokoban도 부분 체크가 크게 올랐다(0/12→8/12). 나머지 8문항은 변화 없거나 미미 — run_python 하나로는 "복잡한 알고리즘 정확성"이나 "엄격한 출력 형식" 같은 남은 요구까지 해결하지 못한다는 것도 함께 드러났다. 나빠진 문항은 없었다.
